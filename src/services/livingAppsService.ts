...
import type { MeineGewohnheiten, TaeglicherCheckIn, CreateMeineGewohnheiten, CreateTaeglicherCheckIn } from '@/types/app';
...
export class LivingAppsService {
  // --- MEINE_GEWOHNHEITEN ---
  static async getMeineGewohnheiten(): Promise<MeineGewohnheiten[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.MEINE_GEWOHNHEITEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as MeineGewohnheiten[];
    return enrichLookupFields(records, 'meine_gewohnheiten');
  }
  static async getMeineGewohnheitenEntry(id: string): Promise<MeineGewohnheiten | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.MEINE_GEWOHNHEITEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as MeineGewohnheiten;
    return enrichLookupFields([record], 'meine_gewohnheiten')[0];
  }
  static async createMeineGewohnheitenEntry(fields: CreateMeineGewohnheiten) {
    return callApi('POST', `/apps/${APP_IDS.MEINE_GEWOHNHEITEN}/records`, { fields: cleanFieldsForApi(fields as any, 'meine_gewohnheiten') });
  }
  static async updateMeineGewohnheitenEntry(id: string, fields: Partial<CreateMeineGewohnheiten>) {
    return callApi('PATCH', `/apps/${APP_IDS.MEINE_GEWOHNHEITEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'meine_gewohnheiten') });
  }
  static async deleteMeineGewohnheitenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.MEINE_GEWOHNHEITEN}/records/${id}`);
  }
  ...
  static async getTaeglicherCheckIn(): Promise<TaeglicherCheckIn[]> {
    ...
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as TaeglicherCheckIn[];
    return enrichLookupFields(records, 'taeglicher_check_in');
  }
  static async getTaeglicherCheckInEntry(id: string): Promise<TaeglicherCheckIn | undefined> {
    ...
    const record = { record_id: data.id, ...data } as TaeglicherCheckIn;
    return enrichLookupFields([record], 'taeglicher_check_in')[0];
  }
  static async createTaeglicherCheckInEntry(fields: CreateTaeglicherCheckIn) {
    return callApi('POST', `/apps/${APP_IDS.TAEGLICHER_CHECK_IN}/records`, { fields: cleanFieldsForApi(fields as any, 'taeglicher_check_in') });
  }
  static async updateTaeglicherCheckInEntry(id: string, fields: Partial<CreateTaeglicherCheckIn>) {
    return callApi('PATCH', `/apps/${APP_IDS.TAEGLICHER_CHECK_IN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'taeglicher_check_in') });
  }
